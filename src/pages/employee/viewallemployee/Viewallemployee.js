...
const Tables = function (props) {
  ...
  const [changedField, setChangedField] = useState({id:'', name:'', email:'', phone:''});
...
useEffect(()=>{
 ...
 dispatch(fetchDomain())
  ...
  const handleEdit= (item) => {
    setModalType('edit')
   setChangedField(prev=>({...prev, id: item._id, name: item.name,email: item.email, phone: item.phone}))
  setModalShow(true)
  }
  ...
  const editSubmit= ()=>{
    ...
    const checkDNS= domainData.filter(e=>(
      e.isVerified && e.domainName===domain
    ))[0]
  ...
  const addSubmit= ()=>{
    ...
    setChangedField({id:'', name:'', email:'', phone:''})
      ...
      toast.error("Please verify the domain first! Then you can add this employee.");
    }
                      ...
                      .map(item => (
                        ...
                        <tr key={item._id}>
                         ...
                         {item.mailsent>0 ? 
                          ...
                          {((item.mailopened/ item.mailsent).toFixed(4)*100)>49? <span className="text-center ml-1 text-white px-4 rounded bg-danger">{((item.mailopened/ item.mailsent).toFixed(4)*100)}%</span>
                           : ((item.mailopened/ item.mailsent).toFixed(4)*100)>15? <span className="text-center ml-1 text-white px-4 rounded bg-warning">{((item.mailopened/ item.mailsent).toFixed(4)*100)}%</span>
                           : <span className="text-center ml-1 text-white px-4 rounded bg-primary">{((item.mailopened/ item.mailsent).toFixed(4)*100)}%</span>
                          }

                          </td>
                          :
                          <td><span className="text-center text-red px-1 rounded bg-light">No Mail Sent</span></td>
